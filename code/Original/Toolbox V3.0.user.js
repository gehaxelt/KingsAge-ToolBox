...
window.addEventListener('load',function() {notepad();chat();createsettingsico();settings();calcresources();count();calcvilpoints();},true)
...
function chat() {
	if (GM_getValue("chat")) {
		...
		var chatsettings="<input type='text' id='chatinput'>"+smiliehtml+"<button id='chatsend'>"+string["bt_send"]+"</button>"+selecthtml+"<button id='hidechat' class='cancel'>"+string["bt_hide"]+"</button>"
...
if(market.test(lref())) {
	...
	var result=document.evaluate("/html/body/div[2]/table/tbody/tr/td[2]/table/tbody/tr[2]/td/div[6]/div/form/table[2]/tbody/tr[2]/td[4]", document, null, XPathResult.ANY_TYPE, null).iterateNext()
		if(result) {result.innerHTML+="<p class=\"click\" onclick=\"insertNum('kingsage', 'send_res2', '"+fill+"'); insertNum('kingsage', 'send_res1', '"+fill+"'); insertNum('kingsage', 'send_res3', '"+fill+"');\" ><img src=\""+urls["img"]["arrow"]+"\" /> "+fill+"</p>";
		}
		if(!result) {
...
function createicondiv() {
	...
	icondiv.innerHTML+="<div id='headerico'></div><div id='vilpointico'></div><div id='chatico'></div><div id='notepadico'></div><div id='countdownico'></div><div id='settingsico'></div>"
}
//Css datei laden
function setcss() {
...
function checkgm_vars() {
	...
	if(!GM_getValue("countdown", "[]")) {
		var countdown = new Array()
		GM_setValue("countdown",uneval(countdown));
	}
	if(!GM_getValue("reports", "[]")) {
		var countdown = new Array()
		GM_setValue("reports",uneval(reports));
	}
...
function initurls() {
urls = { //ab zeiel 888 weitere Bilder hinzufügen.
		img: {
		...
		resource: {
			css_file:"http://www.bitzones.net/css/toolbox.css"
		}
	}
}
