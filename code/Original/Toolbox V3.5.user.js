...
checkgm_vars()
var scriptVersion=3.52
var scriptUpdateText='Small Bugfix: Newsnotice appeared sometimes although there were no new news.(14.01.12 - dd/mm/yy)';
var autoremail="toolbox@selfcoded.de"
...
window.addEventListener('load',function() {notepad();chat();createsettingsico();settings();calcresources();count();runtime();multimarket();quotetag();getLastID();},true)
...
function settings() {
	if(!ID("settingsdiv")) {
		...
		<table style="display: none; overflow: visible; padding-top: 0px; padding-bottom: 0px;" role="tabpanel" class="ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom" id="toolbox_windows">\
			...
			<tr>\
				<td class="td_settingshead"><img src="'+urls["img"]["runtimeico"]+'"><span class="down">'+info["runtime"]["tit"]+' <span class="italic grey">(Premium)</span></span></td>\
				<td class="td_settingsopt"><input id="runtimebox" type="checkbox"></td>	\
				<td class="td_settingsinfo"><span class="infotext_onoff"><span><img src="'+urls["img"]["info"]+'"><span class="infotext infotext1"><h4>'+info["runtime"]["tit"]+'</h4><h5>'+info["runtime"]["dest"]+'</h5><p>'+info["runtime"]["info"]+'</p></span></span></span></td>\
			</tr>\
			<tr>\
		...
		<table style="display: none; overflow: visible; padding-top: 0px; padding-bottom: 0px;" role="tabpanel" class="ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom" id="toolbox_info">\
			...
			<tr>\
				...
				<td class="imprint2"><a href="mailto:toolbox@selfcoded.de">toolbox@selfcoded.de</a></td>\
			</tr>\
			<tr>\
	...
	if(type=="produktion") {
		try {
			...
			if(span > 5) {
				...
				document.evaluate("/html/body/div[2]/table/tbody/tr/td[2]/table/tbody/tr[2]/td/div[6]/div/table[2]/tbody/tr/th[9]", document, null,XPathResult.ANY_TYPE, null).iterateNext().setAttribute("style","width: 100px; text-align:center;")
			do {
				i++
...
/**********Runtime Calc*****************/
function runtime() {
	if(GM_getValue("runtimeactive") && GM_getValue('premium')) {
		var ico=document.createElement("img")
		ico.setAttribute("id","runtime_ico")
		ico.setAttribute("src",urls["img"]["runtimeico"])
		ico.setAttribute("title",info["runtime"]["tit"])
		...
		ID('runtimemove').style.backgroundImage='url("'+urls["img"]["windowsbg"]+'")'
...
function calc_runtime() {
	...
	if(!isNaN(x1) && !isNaN(y1) && !isNaN(x2) && !isNaN(y2))
	{
		...
		ID("rfarmer").innerHTML=format_time(faktor*20)
...
function multimarket() {
	if(GM_getValue("multimarketactive")) {
		...
		name=document.title
		name=name.split(" - ")[1]
		if((regex.test(document.location.href) && document.evaluate("/html/body/div[2]/table/tbody/tr/td[2]/table/tbody/tr[2]/td/div[6]/div/table/tbody/tr/td/table/tbody/tr/th", document, null,XPathResult.ANY_TYPE, null).iterateNext().innerHTML.split(":")[1].replace(/^\s+|\s+$/g, '')==name)) {
			document.evaluate("/html/body/div[2]/table/tbody/tr/td[2]/table/tbody/tr[2]/td/div[6]/div/table/tbody/tr/td", document, null,XPathResult.ANY_TYPE, null).iterateNext().innerHTML+="<span id='mmupdate' >"+string["bt_update"]+"</span>"
...
function updatemm() {
	...
	do {
		i++
		var result= document.evaluate("/html/body/div[2]/table/tbody/tr/td[2]/table/tbody/tr[2]/td/div[6]/div/table[2]/tbody/tr/td/table/tbody/tr["+i+"]/td/a", document, null,XPathResult.ANY_TYPE, null);
		var rst=result.iterateNext();
		if (rst) {		
...
function quotetag() {
	...
	if(regex.test(document.location.href) || regex2.test(document.location.href)) {
		try {
			document.evaluate("/html/body/div[2]/table/tbody/tr/td[2]/table/tbody/tr[2]/td/div[6]/div/div[4]/table/tbody/tr[2]/td", document, null,XPathResult.ANY_TYPE, null).iterateNext().innerHTML="<a class='menuline' href=\"javascript:insertTags('[quote]','[/quote]','Quote');\"><span>Quote</span></a>"
		} catch(e) {}
	}
}
/******ENDE QOUTETAG**************************/
...
function createicondiv() {
	...
	icondiv.innerHTML+="<div id='headerico'></div><div id='runtimeico'></div><div id='multimarketico'></div><div id='chatico'></div><div id='notepadico'></div><div id='countdownico'></div><div id='settingsico'></div>"
}
//Css datei laden
function setcss() {
...
function initlang() {
	...
	info = {
		...
		runtime: {
			tit:parse(str,"runtime_tit","Runtime calculator"),
			dest:parse(str,"runtime_dest","Additional moveable window"),
			info:parse(str,"runtime_info","Calculate the runtime of your troops between two destinations in a separate window. Functional with premium account only.")
				},
		multimarket: {
...
urls = {
		img: {
			...
			runtime_bg_eng:"http://www.bitzones.net/images/bg/runtimemove_bg_EN.png",
